...
| 13. Worker uploads staged output; crashes before result publication. | The object store contains immutable output under the attempt-specific job/task/attempt/assignment key. SQLite still has no authoritative output reference for that object. | Lease expiry or worker-loss handling closes/releases the attempt and may assign a newer generation. Each attempt may conditionally create its own key; only a matching assignment can commit that exact reference to `tasks.result_payload_json`. A late or mis-keyed result cannot replace it. The future orphan collector selects only inactive, non-authoritative staged outputs older than the safety window. | Staged objects from multiple attempts and re-execution: **yes**. More than one authoritative output reference: **no**. | **Existing staging/fencing:** [`ImageConversionProcessorTest#reusesIdenticalAssignmentOutputAndKeepsAttemptsIndependent`](../plugins/conversion/peer/src/test/java/peer/processors/ImageConversionProcessorTest.java), [`ObjectStoreContractTest#putIfAbsentNeverReplacesAnExistingObject`](../taskflow-objectstore-minio/src/test/java/objectstore/minio/ObjectStoreContractTest.java), [`MinioObjectStoreContractTest#separateConversionParticipantsExchangeInputOnlyByPortableObjectKey`](../taskflow-objectstore-minio/src/test/java/objectstore/minio/MinioObjectStoreContractTest.java), and [`DatabaseManagerTest#staleAttemptOutputReferenceCannotReplaceAuthoritativePointer`](../taskflow-persistence-sqlite/src/test/java/server/db/DatabaseManagerTest.java). **Planned cleanup/process-kill:** [`OrphanOutputGcTest#uploadedOutputWithoutResultBecomesCollectable`](FAILURE_MODEL.md#tf-0506--add-orphan-output-garbage-collection) and [`CrashWindowMatrixTest#uploadedOutputBeforeResultIsEventuallyCollected`](FAILURE_MODEL.md#tf-0705--automate-the-crash-window-matrix). |
