...
| 17. RabbitMQ disconnects while work is active. | SQLite contains authoritative accepted work, current assignments/leases, and coordinator outbox rows. RabbitMQ retains durable queued messages and requeues unacknowledged deliveries after connection loss; an in-flight publish may have an unknown outcome. | Consumers and publishers reconnect with bounded concurrent recovery and configured delay/backoff. Broker-held deliveries are redelivered, and pending SQLite outbox rows replay in bounded batches. Lease expiry may create a newer assignment, so old results are fenced. Recovery attempts stop on reconnection or shutdown; message redelivery stops on acknowledgement, permanent rejection, or bounded poison quarantine. | Assignment, result, and final-result delivery and task execution: **yes**. Duplicate authoritative state: **no**. | **Partial:** [`RabbitMqTransportLiveTest#recoversConsumerAndPublisherAfterBrokerSideConnectionDropAgainstLiveBroker`](../taskflow-transport-rabbitmq/src/test/java/transport/rabbitmq/RabbitMqTransportLiveTest.java) proves client recovery, while [`RabbitMqCoordinatorLiveIntegrationTest#connectionLossDuringWorkerAssignmentPublishLeavesOutboxReplayable`](../taskflow-coordinator/src/test/java/server/rabbitmq/RabbitMqCoordinatorLiveIntegrationTest.java) proves the coordinator outbox retains a failed publication and later replays it. **Planned full-outage proof:** [`BrokerRecoveryIntegrationTest#activeWorkCompletesAfterBrokerRestart`](FAILURE_MODEL.md#tf-0306--add-broker-outage-and-restart-recovery). |
| 18. Scheduler mailbox remains full. | Accepted jobs/tasks stay in SQLite. The mailbox is at its configured bound; a not-yet-accepted submission has no accepted SQLite state. | The delivery receives `RETRY_TRANSIENT`; after a confirmed handoff, a TTL retry queue owns the waiting period and the source delivery is acknowledged. A later delivery can enter the mailbox when capacity returns. Persistent saturation is attempt-bounded and ultimately becomes operator-visible quarantine rather than growing memory or spinning CPU. Adaptive intake reduction and fairness under sustained overload remain TF-0406 work. | Broker redelivery: **yes**. Silent replacement of an accepted mailbox item: **no**. Manual redrive may be required after retry exhaustion: **yes**. | **Partial:** [`SchedulerMailboxTest#repeatedBrokerOverflowRequeuesDeliveriesWithoutReplacingAcceptedWork`](../taskflow-core/src/test/java/server/scheduler/SchedulerMailboxTest.java) and [`RabbitMqTransportLiveTest#delaysTransientHandlerRetryWithoutImmediateRedeliverySpinAgainstLiveBroker`](../taskflow-transport-rabbitmq/src/test/java/transport/rabbitmq/RabbitMqTransportLiveTest.java). **Planned:** [`SchedulerOverloadTest#persistentMailboxSaturationPreservesAcceptedWorkAndProgress`](FAILURE_MODEL.md#tf-0406--define-persistent-overload-behavior). |
...
- typed broker delivery classification, bounded delayed retry, automatic final
  ...
  for routes that remain bound while a delayed message returns; recovery when
  an ephemeral participant route disappears during the delay remains part of
  the broker-outage/restart work;
