...
| 18. Scheduler mailbox remains full. | Accepted jobs/tasks stay in SQLite. The mailbox is at its configured bound; a not-yet-accepted submission has no accepted SQLite state. | The delivery receives `RETRY_TRANSIENT`; after a confirmed handoff, a TTL retry queue owns the waiting period and the source delivery is acknowledged. A later delivery can enter the mailbox when capacity returns. The loop drains only `schedulerMessageBatchSize` envelopes before processing a bounded deadline batch, so admitted message pressure cannot exclude lease expiry; messages run first on the next cycle, so due deadlines cannot exclude queued results. Persistent broker saturation remains attempt-bounded and ultimately becomes operator-visible quarantine. Adaptive intake reduction remains TF-0406 work. | Broker redelivery: **yes**. Silent replacement of an accepted mailbox item: **no**. Manual redrive may be required after retry exhaustion: **yes**. | **Partial:** [`SchedulerMailboxTest#repeatedBrokerOverflowRequeuesDeliveriesWithoutReplacingAcceptedWork`](../taskflow-core/src/test/java/server/scheduler/SchedulerMailboxTest.java), [`SchedulerLoopTest#continuousMailboxBacklogStillProcessesDeadlinesEveryCycle`](../taskflow-core/src/test/java/server/scheduler/SchedulerLoopTest.java), [`SchedulerLoopTest#continuousDueDeadlineBacklogCannotStarveQueuedTaskResult`](../taskflow-core/src/test/java/server/scheduler/SchedulerLoopTest.java), and [`RabbitMqTransportLiveTest#delaysTransientHandlerRetryWithoutImmediateRedeliverySpinAgainstLiveBroker`](../taskflow-transport-rabbitmq/src/test/java/transport/rabbitmq/RabbitMqTransportLiveTest.java). **Planned persistent-overload proof:** [`SchedulerOverloadTest#persistentMailboxSaturationPreservesAcceptedWorkAndProgress`](FAILURE_MODEL.md#tf-0406--define-persistent-overload-behavior). |
...
- weighted capacity and explicit configured admission bounds are implemented;
  persistent-overload result/expiry progress and automatic recovery remain
  planned. Deterministic round-robin fairness and capacity-wait reactivation
  are implemented.
