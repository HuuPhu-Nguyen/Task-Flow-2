...
| Task lease/retry service | SQLite task assignment, lease deadline, attempt audit, and distinct task `retry_count` | Deadline and dispatch discovery is index-driven; TF-0402 owns per-cycle batch bounds. A task stops retrying on success, permanent failure, or configured task retry exhaustion. Assignment generation never substitutes for retry count. |
