...
| 20. A valid new submission reaches an admission bound or the pending-outbox count cannot be read. | No J0/T0 job, task, attempt, lease, or outbox row for the candidate has committed. Existing accepted work and pending outbox rows remain authoritative. | Exact owner/hash replay is classified first and returns the existing status or terminal result even while full. A new request that would exceed active jobs, active tasks, submitted-task count, exact inline JSON bytes, or any referenced-payload declaration, or that arrives while the current pending-outbox count is at its threshold, receives a failed protocol-v2 `JOB_RESULT` with typed `admissionRejection`. A count read failure is a storage failure, never an assumed zero. Recovery retains previously accepted work above a newly lowered bound; only new acceptance waits for cleanup. | Submission delivery/retry: **yes**. A durable candidate job/task mutation: **no**. Eviction of accepted work: **no**. | **Existing:** [`SchedulerAdmissionTest`](../taskflow-core/src/test/java/server/scheduler/SchedulerAdmissionTest.java), [`TaskSchedulerPersistenceTest#activeJobAdmissionRejectionPerformsNoSecondDurableCommit`](../taskflow-core/src/test/java/server/scheduler/TaskSchedulerPersistenceTest.java), [`TaskSchedulerPersistenceTest#pendingOutboxCountFailureRejectsWithoutPretendingCountIsZero`](../taskflow-core/src/test/java/server/scheduler/TaskSchedulerPersistenceTest.java), [`DatabaseManagerTest#assignmentCommitBeforePublishLeavesOneDurableIdentityAndPendingOutbox`](../taskflow-persistence-sqlite/src/test/java/server/db/DatabaseManagerTest.java), [`DatabaseManagerTest#pendingOutboxAggregateDistinguishesStorageFailure`](../taskflow-persistence-sqlite/src/test/java/server/db/DatabaseManagerTest.java), [`AdmissionOverloadExperiment#coordinatorHeapPlateausAtConfiguredBounds`](../taskflow-core/src/test/java/server/scheduler/AdmissionOverloadExperiment.java), and the real-broker/schema-v14 [`OverloadExperiment`](../taskflow-coordinator/src/test/java/server/OverloadExperiment.java), documented in [`reports/overload.md`](reports/overload.md). |
| 21. Object storage is unavailable during conversion input upload or executor download. | Before submission publication, any successfully uploaded inputs are immutable non-authoritative staging objects and no J0/T0 state exists. After assignment, SQLite retains the current assignment/lease while the message carries only a portable object key and metadata. | Payload construction fails before publication when a required upload cannot complete and deletes objects from that failed build best-effort. A download outage/missing object fails processor execution through the existing bounded assignment failure/retry path; no local path fallback occurs. Service recovery allows a later explicit submission or permitted assignment retry to open a new client. Referenced-input retention/deletion is not inferred from coordinator task state and remains an explicit non-goal of the output collector. | Upload attempt or task execution after retry: **yes**. A local-filesystem fallback or authoritative object result: **no**. | **Existing process-kill and storage-failure evidence:** [`CrashWindowMatrixTest#partialObjectUploadCannotBecomeAuthoritative`](../taskflow-coordinator/src/test/java/server/CrashWindowMatrixTest.java), [`ConversionClientPluginTest#deletesUploadedInputsWhenBuildFailsAfterStorage`](../plugins/conversion/client/src/test/java/client/plugins/conversion/ConversionClientPluginTest.java), [`ImageConversionProcessorTest#downloadsReferencedInputByObjectKey`](../plugins/conversion/peer/src/test/java/peer/processors/ImageConversionProcessorTest.java), [`MinioObjectStoreContractTest#separateConversionParticipantsExchangeInputOnlyByPortableObjectKey`](../taskflow-objectstore-minio/src/test/java/objectstore/minio/MinioObjectStoreContractTest.java), and [`ObjectStoreContractTest#unavailableStoreIsClassifiedAsStorageFailure`](../taskflow-objectstore-minio/src/test/java/objectstore/minio/ObjectStoreContractTest.java). |
| 22. Immutable object bytes differ from the declared length or SHA-256. | The portable object reference still identifies the expected key, length, digest, and content type. Before publication, an upload has no J0/T0 authority. After assignment, SQLite retains the exact current assignment and its running attempt; corrupt bytes are never a valid task result. | Upload verification rejects and best-effort deletes a just-written mismatched object. Download verification consumes at most the configured bound and rejects truncated, extended, or same-length corrupt bytes before processor/output acceptance. The executor emits `PERMANENT_PAYLOAD_INTEGRITY`; a current coordinator conditionally closes the exact running attempt as terminal, atomically fails the job, emits `payload_integrity_failure_committed`, increments `taskflow_payload_integrity_failures_total`, and creates no replacement assignment. A persistence failure leaves the assignment authoritative for broker redelivery of the same failure result. | Broker/result delivery may repeat until the terminal write commits: **yes**. Processor invocation after failed verification, logical task retry, or corrupt authoritative output: **no**. | **Existing:** [`ObjectStoreIntegrityContractTest#corruptPayloadIsRejectedBeforeProcessorInvocation`](../taskflow-spi/src/test/java/objectstore/ObjectStoreIntegrityContractTest.java), [`ObjectStoreContractTest#putRejectsBytesThatDoNotMatchTheReference`](../taskflow-objectstore-minio/src/test/java/objectstore/minio/ObjectStoreContractTest.java), [`MinioObjectStoreContractTest#corruptObjectBytesAreRejectedBeforeImageProcessing`](../taskflow-objectstore-minio/src/test/java/objectstore/minio/MinioObjectStoreContractTest.java), [`ConversionClientPluginTest#rejectsCorruptObjectResultBeforeWritingOutput`](../plugins/conversion/client/src/test/java/client/plugins/conversion/ConversionClientPluginTest.java), [`PeerExecutionEngineTest#payloadIntegrityFailureIsPermanentAndEmitsStructuredEvent`](../taskflow-core/src/test/java/peer/engine/PeerExecutionEngineTest.java), [`TaskSchedulerFailureTest#permanentPayloadIntegrityFailureTerminatesWithoutReplacementAssignment`](../taskflow-core/src/test/java/server/scheduler/TaskSchedulerFailureTest.java), and [`TaskSchedulerPersistenceTest#payloadIntegrityFailurePersistsTerminalAttemptWithoutRetry`](../taskflow-core/src/test/java/server/scheduler/TaskSchedulerPersistenceTest.java). |
| 23. SQLite loses write capability while the coordinator process remains alive. | Previously accepted jobs, tasks, attempts, leases, terminal results, and pending outbox rows remain in SQLite; no health probe commits a state change. A candidate new job has no J0/T0 authority. | Liveness remains `UP`, while readiness becomes `DEGRADED` with `SQLITE_NOT_WRITABLE`. The rollback-only write probe exercises the real writer path without committing its no-op update. Exact accepted submission replay is classified before the new-job gate when durable inspection is available; genuinely new work is rejected before in-memory acceptance. Restoring SQLite write capability makes the next probe ready automatically. | Health probes and rejected submission delivery: **yes**. A durable probe mutation or degraded new-job acceptance: **no**. | **Existing:** [`DatabaseManagerTest#writabilityProbeRollsBackAndRecoversAfterInjectedWriteFailure`](../taskflow-persistence-sqlite/src/test/java/server/db/DatabaseManagerTest.java), [`CoordinatorReadinessProbeTest#actualSqliteOutboxAndSchedulerSignalsRecoverWithoutRestart`](../taskflow-coordinator/src/test/java/server/health/CoordinatorReadinessProbeTest.java), [`CoordinatorHealthTest#dependencyLossDegradesAndRecoveryRestoresReadinessWithoutRestart`](../taskflow-coordinator/src/test/java/server/health/CoordinatorHealthTest.java), and [`SchedulerAdmissionTest#degradedCoordinatorRejectsNewJobBeforeInMemoryAcceptance`](../taskflow-core/src/test/java/server/scheduler/SchedulerAdmissionTest.java). |
...
- `DuplicateSubmissionIntegrationTest#lostAcceptanceResponseReplaysAcceptedJob`
  ...
  `CrashWindowMatrixTest#acceptedJobSurvivesLostAcceptanceResponse` supplies
  the corresponding OS-process-kill boundary.
...
- The earlier combined name
  ...
  live retry mapping. The OS-process matrix separately covers the
  confirmed-but-unobserved result-publication window in row 7.
...
- `OrphanOutputGcTest#uploadedOutputWithoutResultBecomesCollectable` uploads
  under an attempt key, omits result publication, advances an injected clock
  beyond the safety window, and deletes only the non-authoritative object.
- `OrphanOutputGcTest#staleAttemptIsDeletedWhileActiveAndAuthoritativeOutputsArePreserved`
  proves the three SQLite classifications in one bounded pass.
- `OrphanOutputGcTest#objectStoreOutageRecordsFailureAndLaterBoundedBatchRetriesIt`
  plus
  `DatabaseManagerTest#deletionFailureRetryStateSurvivesRestartAndClearsIdempotently`
  prove outage deferral and durable idempotent retry.
...
runs all behavioral methods unchanged through
[`SqlitePersistenceContractTest`](../taskflow-persistence-sqlite/src/test/java/server/db/SqlitePersistenceContractTest.java):

- [`PersistenceContractTest#acceptedJobSurvivesCoordinatorRestart`](../taskflow-core/src/test/java/server/db/PersistenceContractTest.java)
  reopens the store after committed acceptance and reconstructs the same
  job/tasks.
- [`PersistenceContractTest#matchingAssignmentCommitsExactlyOnce`](../taskflow-core/src/test/java/server/db/PersistenceContractTest.java)
  submits a matching result twice and observes one commit plus one typed
  duplicate.
- [`PersistenceContractTest#restartPreservesUnexpiredLease`](../taskflow-core/src/test/java/server/db/PersistenceContractTest.java)
  attempts release with injected time before expiry and retains the exact
  assignment identity and running attempt.
- [`PersistenceContractTest#restartReleasesExpiredLeaseAndFencesOldResult`](../taskflow-core/src/test/java/server/db/PersistenceContractTest.java)
  recovers at exact expiry, creates the next generation, rejects the old
  result, and commits the current result.

Ten additional inherited methods cover atomic job/task and assignment/outbox
creation, idempotent submission conflicts, generation advancement, persisted
retry, stale classification, terminal monotonicity, final-result outbox
commit, previous-schema migration, and durable outbox failure/publish/reopen
state. SQLite-specific trigger-fault and concurrency tests remain linked in
the matrix above; the separate process crash-window suite exercises the
cross-component kill seams.
...
now starts a separate child JVM for every named failpoint, waits for the
boundary signal, kills that operating-system process forcibly, and verifies
recovery from the surviving SQLite/RabbitMQ/MinIO state in the parent.

The nine stable methods cover job commit/lost response, assignment/outbox
commit, assignment confirm before sent marking, uncertain task-result
publication, authoritative result commit before finalization, terminal
job/final-outbox commit, final-result confirm before sent marking, interrupted
object streaming, and completed attempt-output upload before result
publication. The managed suite uses RabbitMQ and MinIO Testcontainers plus a
stable Toxiproxy AMQP endpoint. See the
[process crash-window report](CRASH_WINDOW_MATRIX.md) for the exact mapping,
command, bounds, and limitations.
...
- lost-response submission replay now uses the schema-v12 canonical request
  hash and requester-owner tuple, including an OS-process kill immediately
  after the job transaction commits;
...
- typed broker delivery classification, bounded delayed retry, automatic final
  ...
  offline;
- broker publication remains at least once; the process crash harness proves
  the named publication/mark windows while retaining exact durable identities;
- the MinIO/S3 streaming port, adapter contract, local Compose environment,
  runtime references, integrity checks, immutable attempt output staging,
  authoritative pointer commitment, and bounded orphan-output collection are
  implemented. Interrupted upload and upload-before-result process windows are
  proved; automatic input-object retention/deletion remains separate scope;
