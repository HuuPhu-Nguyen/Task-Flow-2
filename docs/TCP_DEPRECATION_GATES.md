...
| JavaFX RabbitMQ desktop smoke or automation evidence covers submit, assigned-task execution, `JOB_RESULT` reception, result handling, disconnect, and broker-failure handling. | Procedure added; evidence still open. | `docs/GUI_RABBITMQ_DESKTOP_SMOKE.md` and `scripts/smoke-rabbitmq-gui.ps1` provide a repeatable manual smoke path. | Record one successful desktop run, or add stable JavaFX automation with a display backend. |
