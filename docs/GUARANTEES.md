...
| <a id="e-i9"></a> I9 | Partial; the streaming port, controlled-key boundary, required metadata, and MinIO adapter contract exist | [`ObjectStoreContractTest#putStreamsContentAndRoundTripsRequiredMetadata`](../taskflow-objectstore-minio/src/test/java/objectstore/minio/ObjectStoreContractTest.java); [`MinioObjectStoreContractTest#missingRequiredDigestMetadataIsClassified`](../taskflow-objectstore-minio/src/test/java/objectstore/minio/MinioObjectStoreContractTest.java); [`ObjectStoreArchitectureTest#frameworkPortAndCoreRemainIndependentFromMinioSdk`](../taskflow-spi/src/test/java/objectstore/ObjectStoreArchitectureTest.java) | `ObjectStoreIntegrityContractTest#corruptPayloadIsRejectedBeforeProcessorInvocation` (TF-0504) | Wire `ObjectReference` into runtime payload messages and verify exact streamed length/SHA-256 before processor invocation or authoritative output acceptance. Metadata round-trip alone does not prove content integrity. |
