...
Object-backed conversion uploads verify the streamed bytes against their
portable reference. Executor input downloads and requester result downloads
verify exact length and digest before processor or output-file acceptance.
Immutable corruption is reported as a permanent classified task failure, so a
current coordinator closes the exact assignment durably and terminally without
creating a retry generation. See [E-I9](#e-i9).
