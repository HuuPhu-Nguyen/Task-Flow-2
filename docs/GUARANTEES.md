...
The opt-in
[`OverloadExperiment`](../taskflow-coordinator/src/test/java/server/OverloadExperiment.java)
adds a real-broker/schema-v14 profile with exact typed admission, queue,
durable-progress, retained-heap, drain, and no-restart recovery evidence in
[`docs/reports/overload.md`](reports/overload.md).
...
Existing bounded chaos evidence:
[E-I10 — `CorrectnessChaosExperiment#allJobsTerminateAfterFailuresStop`](#e-i10)
and the [100,000-task correctness-chaos report](reports/correctness-chaos.md).
...
- Recovery makes progress only under the I10 assumptions. The
  ...
  implemented. The bounded process-crash matrix and seeded 100,000-task
  correctness-chaos experiment are also implemented; cluster failover and
  multi-coordinator recovery remain outside their evidence.
...
| <a id="e-i10"></a> I10 | Existing for deterministic cross-job, bounded generated, single-broker restart, process-crash, and seeded 100,000-task chaos evidence | [`TaskSchedulerFailureTest#taskFailureAtRetryLimitReturnsFailedJobResult`](../taskflow-core/src/test/java/server/scheduler/TaskSchedulerFailureTest.java); [`TaskSchedulerFailureTest#taskTimeoutAtRetryLimitReturnsFailedJobResult`](../taskflow-core/src/test/java/server/scheduler/TaskSchedulerFailureTest.java); [`AssignmentServiceBatchTest#oneLargeJobAndTenSmallJobsAllDispatchInTheirFirstCompleteRound`](../taskflow-core/src/test/java/server/scheduler/AssignmentServiceBatchTest.java); [`AssignmentServiceBatchTest#timedRecheckRestoresWaitingJobsWithoutWallClockSleep`](../taskflow-core/src/test/java/server/scheduler/AssignmentServiceBatchTest.java); [`CoordinatorStartupRecoveryTest#releasesExpiredAssignedLeasesOnResume`](../taskflow-coordinator/src/test/java/server/CoordinatorStartupRecoveryTest.java); [`JobFinalizationCrashTest#lastTaskCommitCannotStrandJob`](../taskflow-coordinator/src/test/java/server/JobFinalizationCrashTest.java); [`RabbitMqBrokerRecoveryIntegrationTest#unavailableStartupAndActiveBrokerRestartRecoverOutboxConsumersAndFencing`](../taskflow-coordinator/src/test/java/server/rabbitmq/RabbitMqBrokerRecoveryIntegrationTest.java); [`TaskFlowModelPropertyTest#generatedSequencesPreserveDurableSchedulerProperties`](../taskflow-coordinator/src/test/java/server/TaskFlowModelPropertyTest.java); [`CorrectnessChaosExperiment#allJobsTerminateAfterFailuresStop`](../taskflow-coordinator/src/test/java/server/CorrectnessChaosExperiment.java); [100,000-task correctness-chaos report](reports/correctness-chaos.md) | — | The chaos run covers one seed, one coordinator, one broker host, and lightweight in-JVM executor fixtures; clustered broker failover and multi-coordinator authority remain outside scope. |

The process suite additionally links I9 to
[`CrashWindowMatrixTest#partialObjectUploadCannotBecomeAuthoritative`](../taskflow-coordinator/src/test/java/server/CrashWindowMatrixTest.java)
and
[`#uploadedOutputBeforeResultIsEventuallyCollected`](../taskflow-coordinator/src/test/java/server/CrashWindowMatrixTest.java),
and links I10's last-result recovery precondition to
[`#resultCommitBeforeFinalizationRecoversTerminalResult`](../taskflow-coordinator/src/test/java/server/CrashWindowMatrixTest.java).

The reusable persistence suite also adds adapter-neutral evidence for I3
through
[`PersistenceContractTest#staleResultIsClassifiedWithoutReplacingCurrentAssignment`](../taskflow-core/src/test/java/server/db/PersistenceContractTest.java)
and
...
for I5 through
[`#assignmentAndOutboxCommitAtomically`](../taskflow-core/src/test/java/server/db/PersistenceContractTest.java),
[`#finalJobAndFinalResultOutboxCommitAtomically`](../taskflow-core/src/test/java/server/db/PersistenceContractTest.java),
and
[`#outboxReplayStateTransitionsSurviveRestart`](../taskflow-core/src/test/java/server/db/PersistenceContractTest.java);
for I6 through
[`#duplicateSubmissionIsIdempotentAndClassified`](../taskflow-core/src/test/java/server/db/PersistenceContractTest.java);
and for I10's durable recovery preconditions through
[`#restartPreservesUnexpiredLease`](../taskflow-core/src/test/java/server/db/PersistenceContractTest.java).
All methods run unchanged in the SQLite binding. Generated event sequences,
process-kill windows, and end-to-end chaos are separate evidence layers rather
than persistence-adapter contract responsibilities.

The bounded
[`TaskFlowModelPropertyTest`](../taskflow-coordinator/src/test/java/server/TaskFlowModelPropertyTest.java)
now supplies the generated-event layer for I2, I3, I4, I5, I6, the
capacity-projection part of I7, and I10's restored-infrastructure assumption.
It compares an independent test-only reference model with the real scheduler,
SQLite store, startup recovery, capacity ledger, and outbox replayer after
every event. The exact seed set, bounds, command, and limits are recorded in
the [model-based testing evidence](MODEL_BASED_TESTING.md). The separate
[process crash-window matrix](CRASH_WINDOW_MATRIX.md) supplies the OS-process
layer. The
[100,000-task correctness-chaos experiment](reports/correctness-chaos.md)
supplies the seeded mixed-failure layer while explicitly retaining its
single-coordinator, single-broker, and lightweight-fixture limitations.
