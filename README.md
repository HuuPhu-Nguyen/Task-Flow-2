...
- SQLite is the current `JobStateStore` implementation. Its schema is versioned, task rows enforce job referential integrity, initial job persistence failures reject job startup, post-start task-state persistence failures fail jobs terminally, and terminal job-status write failures after result delivery are logged as degraded history. Schema-v2 task payload/result snapshots allow coordinator startup to resume rebuildable `RUNNING` jobs. Assigned tasks are reset to pending on resume because task leases are not implemented; legacy or otherwise non-resumable running jobs are marked failed. PostgreSQL/Flyway and lease-based recovery are still planned for production-style state management.
