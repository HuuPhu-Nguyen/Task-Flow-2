...
acknowledgement, five typed delivery dispositions, publisher confirms, and
at-least-once delivery. With SQLite,
...
production-ready. Managed single-broker outage/restart recovery is automated;
the remaining operational gates are documented below.
...
`docs/BACKPRESSURE_SCOPE.md` for overload boundaries,
`docs/OBSERVABILITY.md` for the coordinator event schema, and
`docs/OBSERVABILITY_SCOPE.md` for the current event inventory and metrics.
...
- `taskflow-objectstore-minio` - MinIO/S3-compatible adapter for the framework-owned streaming `ObjectStore` port; the MinIO SDK remains isolated here
...
Framework core no longer imports concrete image, video, text, or example job classes. New task types should be added under `plugins/<domain>` with separate model, server, client, and peer artifacts when a role needs different dependencies. Coordinator-side scheduling uses `server.job.TaskPlugin`, executor-role processing uses `peer.engine.PeerProcessorPlugin`, and requester-role upload/final-result handling uses `client.ClientJobPlugin`. Providers are registered under `META-INF/services`. Server plugins validate submitted parameters and payload shapes during job startup, so malformed submissions fail with a terminal `JOB_RESULT` before tasks are persisted or assigned. They also validate plugin-owned task results before the coordinator attempts the authoritative SQLite commit. The reusable `PluginContractTest` fixture runs unchanged for example, text, image, and video bindings and proves deterministic splitting/aggregation, stable task identifiers, payload/result validation, retry/resource agreement, role dependency separation, and portable object references where applicable. Conversion plugins keep only small file bytes inline as Base64; inputs at or above the configured inline limit are uploaded to object storage and sent as portable `ObjectReference` metadata. Local/shared-filesystem references are rejected. See `docs/PLUGIN_AUTHORING.md` for the contributor checklist and role-by-role contract, `docs/PAYLOAD_STORAGE.md` for payload-reference ownership and limits, and `docs/PEER_LIFECYCLE.md` for the participant requester/result-handling lifecycle.
...
credentials. Participant and coordinator runtimes discover the MinIO provider
without adding the SDK to coordinator/core/persistence source. Conversion submitters upload
...
is only staged data. The coordinator periodically scans timestamped immutable
attempt outputs in bounded batches, preserves exact active/authoritative keys
through SQLite classification, deletes old orphans idempotently, and persists
...
`inspect` and `inspect-quarantine` are non-destructive. `redrive` republishes only valid TaskFlow broker envelopes to the original routing key captured by RabbitMQ dead-letter metadata, increments `x-taskflow-redrive-count`, and acknowledges the DLQ entry only after the broker confirms the publish. `redrive-quarantine` applies the same confirmation rule to the final quarantine queue and resets `x-taskflow-delivery-attempt` to `1`, giving the message a fresh bounded retry budget while retaining its failure metadata. Malformed or unknown-route entries are not redriven; use `quarantine` or `discard` after review.
...
RabbitMQ executor roles execute assignments asynchronously relative to broker delivery callbacks. Assignment acknowledgements are deferred until the participant publishes the corresponding `TASK_RESULT`. For CLI-submitted jobs, `JOB_RESULT` acknowledgement is deferred until the requester role has handled the result locally; for JavaFX participants, `JOB_RESULT` is acknowledged after the GUI accepts it for routing to the user-facing result flow. Successful handling uses `ACK_SUCCESS`; duplicate or stale work uses `ACK_DUPLICATE_OR_STALE`; validation and compatibility failures use `REJECT_INVALID`; deterministic processing failures use `QUARANTINE_POISON`; and only temporary infrastructure or bounded-mailbox pressure uses `RETRY_TRANSIENT`. Both retry-eligible failure dispositions use the configured bounded TTL schedule and enter final quarantine after exhaustion; validation rejection remains immediate and never enters that retry loop. RabbitMQ publishes wait for broker publisher confirms before returning success. Peer-targeted compatibility routes also use mandatory-return detection. With SQLite persistence, coordinator-originated task assignments and final job results are written to the broker outbox before publish and must carry a peer route. The coordinator marks an outbox row sent only after the persistent publish is confirmed and mandatory-return processing determines that peer routing succeeded. A connection failure, nack, timeout, mandatory return, or failed SQLite sent mark leaves the row pending. A failed sent mark after broker acceptance therefore replays the exact envelope and can duplicate delivery; assignment-generation fencing and terminal-state monotonicity protect authoritative state rather than claiming exactly-once delivery. If the coordinator scheduler mailbox is full, RabbitMQ job submissions and task results receive the transient-retry disposition instead of being accepted into process memory.
...
.\mvnw.cmd -pl taskflow-coordinator -am "-Dtaskflow.rabbitmq.live=true" "-Dtest=RabbitMqCoordinatorShutdownLiveIntegrationTest" "-Dsurefire.failIfNoSpecifiedTests=false" test
.\mvnw.cmd -pl taskflow-coordinator -am "-Dtaskflow.rabbitmq.live=true" "-Dtest=RabbitMqBrokerRecoveryIntegrationTest" "-Dsurefire.failIfNoSpecifiedTests=false" test
```

Alternatively, set `TASKFLOW_RABBITMQ_LIVE_TEST=true` before running the same tests.

GitHub Actions runs a `Push-integration` job with a
`rabbitmq:3.13-management` service container. It combines the SQLite
migration/persistence contract, real MinIO contract, reusable
RabbitMQ/Testcontainers contract, non-chaos live/integration suites, and the
command-line/GUI broker adapters:

```bash
bash ./mvnw --batch-mode --no-transfer-progress -pl taskflow-persistence-sqlite,taskflow-objectstore-minio,taskflow-transport-rabbitmq,taskflow-coordinator,taskflow-peer,taskflow-gui -am -Dtaskflow.rabbitmq.live=true -Dtest='*IntegrationTest,*LiveTest,DatabaseManagerTest,SqlitePersistenceContractTest,MinioObjectStoreContractTest,RabbitMqBrokerContractTest,PeerNodeTest,RabbitMqJobSubmissionClientTest,RabbitMqCoordinatorConnectionTest,!RabbitMqBrokerRecoveryIntegrationTest' -Dsurefire.failIfNoSpecifiedTests=false test
```

The coordinator managed broker-restart test and process crash-window matrix
run in scheduled chaos with expanded model sequences and a reduced seeded
mixed-failure workload. Report-grade correctness and scaling runs are manual
and require the documented benchmark runner. See
[`docs/CI_EVIDENCE_TIERS.md`](docs/CI_EVIDENCE_TIERS.md) for all four
triggers, selectors, artifacts, local commands, retention, and limitations.

The transport live tests create unique non-durable exchanges and queues, validate shared-route delivery, validate peer-specific delivery, verify shared-route acknowledgement drains the queue, verify delayed transient-handler retry, verify deterministic poison reaches final quarantine after the exact configured attempt bound without immediate-redelivery spin, verify preserved attempt/reason/original-route metadata and manual quarantine redrive, verify ordinary DLQ inspect/redrive/quarantine behavior for rejected invalid or malformed messages, verify `prefetch=1` limits unacknowledged deliveries, verify client recovery after a broker-side connection close, and clean up their broker resources.
...
This command and the full `.\mvnw.cmd test` gate require a working Docker
...
- The object-store port and MinIO adapter contract are implemented, including
  ...
  The coordinator now performs safety-window and batch-bounded orphan-output
...
- Multi-node RabbitMQ failover and participant-side durable result replay if the deployment goal requires them
...
- Docker Desktop or Docker Engine is installed for the MinIO-backed Maven
  contract test, the local RabbitMQ broker, or the one-command RabbitMQ demo.
