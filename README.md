...
The transport live tests create unique non-durable exchanges and queues, validate shared-route delivery, validate peer-specific delivery, verify shared-route acknowledgement drains the queue, verify transient-handler retry, verify deterministic-handler reject-to-dead-letter behavior, verify DLQ inspect/redrive/quarantine behavior for rejected handler failures and malformed poison messages, verify `prefetch=1` limits unacknowledged deliveries, verify client recovery after a broker-side connection close, and clean up their broker resources.
