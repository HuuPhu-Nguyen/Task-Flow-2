...
- RabbitMQ is the planned primary runtime, but RabbitMQ mode is still transitional. Implemented pieces include peer-specific routing, explicit peer IDs, peer-scoped job IDs, persisted peer registry metadata, publisher confirms, coordinator outbox replay with live crash-window coverage, command-line and JavaFX GUI submit/result handling, and DLQ inspect/redrive/quarantine/discard commands. Open gaps include broker-backed CI, JavaFX desktop end-to-end automation, broader broker outage behavior, and default-flip evidence. `docs/PEER_IDENTITY.md` records the current identity contract; `docs/RUNTIME_STRATEGY.md` records the target-runtime decision and TCP deprecation/removal gates; `docs/RABBITMQ_SCOPE.md` records the current support limits.
...
- RabbitMQ primary-runtime gates: broker-backed CI, broader broker-failure coverage, JavaFX desktop automation evidence, and docs/demo alignment
